...
const Dropdown = ({ handleCompetition, league }) => {
  ...
  return (
    <div className="relative">
      <div className="flex bg-white justify-between text-sm leading-5 py-4 my-4 px-2 font-medium text-gray-700 items-center shadow-sm">
        <div className="w-2/3">{league} Matchday Results</div>
        <div
      ...
      >
        ...
        >
          {competitions
            .filter(
              ({type, id}) => type === "league" && id !== 2777 && id !== 2771 
            )
            .map(({ id, name, slug }) => (
